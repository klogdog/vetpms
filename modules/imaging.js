...
class VetPMSImaging {
  ...
  initEventListeners() {
    ...
    this.caliperBtn.addEventListener('click', () => {
      ...
      window.vetApp.showToast(
        this.caliperEnabled
          ? 'VHS Caliper Tools enabled. Click & drag handles to align.'
          : 'Caliper Tools disabled.',
        'imaging'
      );
      this.renderViewport();
    });
  ...
  renderViewport() {
    ...
    if (!this.activeOrderId) {
      ...
      this.simSub.innerText =
        'Order a radiograph in SOAP notes or choose a pending MWL order to acquire.';
  ...
  simulateAcquisition() {
    ...
    setTimeout(() => {
      ...
      if (p) {
        ...
        window.vetApp.showToast(
          `DICOM C-STORE successful! Radiograph attached to <b>${p.name}</b>'s chart.`,
          'success'
        );
      }
  ...
  drawCaliperOverlay(ctx) {
    if (!this.caliperEnabled) {
      document.getElementById('overlay-clinic-left').innerHTML =
        'VETPMS MEDICAL PACS<br>CALIPER: OFF';
      return;
    }
    ...
    // Handles
    ctx.beginPath();
    ctx.arc(la.p1.x, la.p1.y, 6, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(la.p2.x, la.p2.y, 6, 0, Math.PI * 2);
    ctx.fill();
    ...
    // Handles
    ctx.beginPath();
    ctx.arc(sa.p1.x, sa.p1.y, 6, 0, Math.PI * 2);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(sa.p2.x, sa.p2.y, 6, 0, Math.PI * 2);
    ctx.fill();
