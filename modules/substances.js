...
class VetPMSSubstances {
  ...
  injectASAPExporterPanel() {
    ...
    panel.style =
      'margin-bottom: 20px; background: rgba(139, 92, 246, 0.05); border-color: rgba(139, 92, 246, 0.25);';
    panel.innerHTML = `
    ...
    document
      .getElementById('btn-generate-asap')
      .addEventListener('click', () => this.generateASAP42BText());
  }
  ...
  renderInventory() {
    ...
    vault.forEach((drug) => {
      ...
      let containersHTML = drugContainers
        .map(
          (c) => `
  ...
  handleSubstanceWithdrawal() {
    ...
    if (totalDrawn > drug.balance) {
      window.vetApp.showToast(
        `Insufficient vault stock! <b>${drug.name}</b> balance is ${drug.balance.toFixed(1)}mL.`,
        'danger'
      );
      return;
    }
    ...
    if (witnessPIN === '1234') {
      ...
      window.vetApp.showToast(
        `DEA SECURITY DECLINED: Witness PIN signature invalid! Log rejected.`,
        'danger'
      );
      return;
    }
    ...
    const containers = window.vetApp.state.drugContainers.filter(
      (c) => c.drugId === drugId && !c.depleted
    );
    ...
    for (let c of containers) {
      ...
      if (c.volume <= 0) {
        ...
        c.volume = 0;
        window.vetApp.showToast(
          `DEA Vault Alert: Vial container <b>${c.containerId}</b> has been depleted!`,
          'warning'
        );
      }
    }
    ...
    window.vetApp.showToast(
      `Vault dispense logged. <b>${amount.toFixed(1)}mL</b> of ${drugBaseName} drawn for ${patient.name}.`,
      'success'
    );
  ...
  generateASAP42BText() {
    ...
    logs.forEach((l, idx) => {
      ...
      const p = window.vetApp.state.animals.find(
        (a) => a.name.toLowerCase() === l.patientName.toLowerCase()
      );
      const owners = window.vetApp.getOwnershipDetails(p ? p.id : '');
      const primaryOwner = owners.find((o) => o.billing) || owners[0] || {};

      const ownerLastName = primaryOwner.ownerName
        ? primaryOwner.ownerName.split(' ').pop().toUpperCase()
        : 'UNKNOWN';
      const ownerFirstName = primaryOwner.ownerName
        ? primaryOwner.ownerName.split(' ')[0].toUpperCase()
        : 'UNKNOWN';
